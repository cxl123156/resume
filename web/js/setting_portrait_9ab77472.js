        ...
        var w, N, C, k, E, S, A, j, D, L, H, M, q, _, F, O, B, P = "sizzle" + -new Date,
            ...
            Z = J.indexOf ||
                function(e) {
                    ...
                    return -1
                }, et = "[\ \\t\\r\\n\\f]", tt = "(?:\\\\.|[\\w-]|[^\-\ ])+", nt = tt.replace("w", "w#"), rt = "([*^$|!~]?=)", it = "\\[" + et + "*(" + tt + ")" + et + "*(?:" + rt + et + "*(?:(['\"])((?:\\\\.|[^\\\\])*?)\\3|(" + nt + ")|)|)" + et + "*\\]", ot = ":(" + tt + ")(?:\\(((['\"])((?:\\\\.|[^\\\\])*?)\\3|((?:\\\\.|[^\\\\()[\\]]|" + it.replace(3, 8) + ")*)|.*)\\)|)", at = RegExp("^" + et + "+|((?:^|[^\\\\])(?:\\\\.)*)" + et + "+$", "g"), ut = RegExp("^" + et + "*," + et + "*"), lt = RegExp("^" + et + "*([\ \\t\\r\\n\\f>+~])" + et + "*"), ct = RegExp(ot), ft = RegExp("^" + nt + "$"), pt = {
                ID: RegExp("^#(" + tt + ")"),
                CLASS: RegExp("^\\.(" + tt + ")"),
                NAME: RegExp("^\\[name=['\"]?(" + tt + ")['\"]?\\]"),
            ...
            }, dt = /[ \t\r\n\f]*[+~]/, ht = /\{\s*\[native code\]\s*\}/, gt = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/, mt = /^(?:input|select|textarea|button)$/i, yt = /^h\d$/i, vt = /'|\\/g, bt = /\=[ \t\r\n\f]*([^'"\]]*)[ \t\r\n\f]*\]/g, xt = /\\([\da-fA-F]{1,6}[ \t\r\n\f]?|.)/g, Tt = function(e, t) {
                var n = "0x" + t - 65536;
...
(function(a) {
    a.Jcrop = function(f, ak) {
        var af = a.extend({}, a.Jcrop.defaults),
            F, X = navigator.userAgent.toLowerCase(),
            d = /msie/.test(X),
            U = /msie [1-6]\./.test(X);
        ie7mode = /msie [7]\./.test(X);

        function z(az) {
            return Math.round(az) + "px"
        }
        function V(az) {
            return af.baseClass + "-" + az
        }
        function ab() {
        ...
        function aw(az) {
            var aA = a(az).offset();
            return [aA.left, aA.top]
        }
        function ap(az) {
            return [(az.pageX - F[0]), (az.pageY - F[1])]
        }
        function ag(az) {
            if (typeof(az) !== "object") {
                az = {}
            ...
            af = a.extend(af, az);
            a.each(["onChange", "onSelect", "onRelease", "onDblClick"], function(aA, aB) {
                if (typeof(af[aB]) !== "function") {
                    af[aB] = function() {}
                }
            })
        }
        function v(aB, aE, aD) {
            F = aw(R);
            aj.setCursor(aB === "move" ? aB : aB + "-resize");
            if (aB === "move") {
                return aj.activateHandlers(ao(aE), s, aD)
            }
            var az = k.getFixed();
            var aA = Q(aB);
            var aC = k.getCorner(Q(aA));
            k.setPressed(k.getCorner(aA));
            k.setCurrent(aC);
            aj.activateHandlers(D(aB, az), s, aD)
        }
        function D(aA, az) {
            return function(aB) {
                if (!af.aspectRatio) {
        ...
        function ao(aA) {
            ...
            T.watchKeys();
            return function(aB) {
                k.moveOffset([aB[0] - az[0], aB[1] - az[1]]);
                az = aB;
                ax.update()
            }
        }
        ...
        function x(aD, aA, aC) {
            ...
            if ((az > aA) && aA > 0) {
                az = aA;
                aB = (aA / aD.width()) * aD.height()
            }
            if ((aB > aC) && aC > 0) {
                aB = aC;
                az = (aC / aD.height()) * aD.width()
            }
            aa = aD.width() / az;
            ai = aD.height() / aB;
            aD.width(az).height(aB)
        }
        function q(az) {
            return {
                x: az.x * aa,
        ...
        function s(aA) {
            var az = k.getFixed();
            if ((az.w > af.minSelect[0]) && (az.h > af.minSelect[1])) {
                ax.enableHandles();
                ax.done()
            } else {
                ax.release()
            }
            aj.setCursor(af.allowSelect ? "crosshair" : "default")
        }
        function o(az) {
            if (af.disabled) {
                return false
            ...
            F = aw(R);
            ax.disableHandles();
            aj.setCursor("crosshair");
            var aA = ap(az);
            k.setPressed(aA);
            ax.update();
            aj.activateHandlers(b, s, az.type.substring(0, 5) === "touch");
            T.watchKeys();
            az.stopPropagation();
            az.preventDefault();
            return false
        ...
        function at() {
            var az = a("<div></div>").addClass(V("tracker"));
            if (d) {
                az.css({
                    opacity: 0,
        ...
        if (f.tagName == "IMG") {
            ...
            var R = u.clone().removeAttr("id").css(c).show();
            R.width(u.width());
            R.height(u.height());
            u.after(R).hide()
        } else {
        ...
        y = "-" + (n / 2 + "px");
        Y = "-" + (l / 2 + "px");
        t = a("<div />").width(n).height(l).addClass(V("holder")).css({
            position: "relative",
            left: ae,
            top: H,
            marginLeft: y,
        ...
        }).insertAfter(u).append(R);
        if (af.addClass) {
            t.addClass(af.addClass)
        }
        var ad = a("<div />"),
            ay = a("<div />").width("100%").height("100%").css({
                zIndex: 310,
                position: "absolute",
                overflow: "hidden"
            }),
            Z = a("<div />").width("100%").height("100%").css("zIndex", 320),
            M = a("<div />").css({
                position: "absolute",
                zIndex: 600
            }).dblclick(function() {
                var az = k.getFixed();
                af.onDblClick.call(E, az)
            }).insertBefore(R).append(ay, Z);
        if (K) {
            ad = a("<img />").attr("src", R.attr("src")).css(c).width(n).height(l), ay.append(ad)
        }
        if (U) {
            M.css({
                overflowY: "hidden"
        ...
        var P = af.boundary;
        var G = at().width(n + (P * 2)).height(l + (P * 2)).css({
            position: "absolute",
            top: z(-P),
            left: z(-P),
            zIndex: 290
        ...
        var av = af.bgColor,
            C = af.bgOpacity,
            e, A, m, p, aa, ai, i = true,
            g, r, au;
        F = aw(R);
        var ar = (function() {
            function az() {
                var aG = {},
                    aE = ["touchstart", "touchmove", "touchend"],
                ...
                try {
                    for (aD = 0; aD < aE.length; aD++) {
                        ...
                        var aC = (aB in aF);
                        if (!aC) {
                            aF.setAttribute(aB, "return;");
                            aC = typeof aF[aB] == "function"
                        }
                        aG[aE[aD]] = aC
                    }
                    return aG.touchstart && aG.touchend && aG.touchmove
                } catch (aH) {
                    return false
                }
            }
            function aA() {
                if ((af.touchSupport === true) || (af.touchSupport === false)) {
                    return af.touchSupport
                } else {
                    return az()
                }
            }
            return {
                createDragger: function(aB) {
                    return function(aC) {
                        if (af.disabled) {
                            return false
                        }
                        if ((aB === "move") && !af.allowMove) {
                            return false
                        }
                        F = aw(R);
                        g = true;
                        v(aB, ap(ar.cfilter(aC)), true);
                        aC.stopPropagation();
                        aC.preventDefault();
                        return false
                    }
                },
                newSelection: function(aB) {
                    return o(ar.cfilter(aB))
                },
                cfilter: function(aB) {
                    aB.pageX = aB.originalEvent.changedTouches[0].pageX;
                    aB.pageY = aB.originalEvent.changedTouches[0].pageY;
                    return aB
                },
                isSupported: az,
                support: aA()
        ...
        var k = (function() {
            ...
            function aD(aP) {
                ...
                return [Math.round(aP[0]), Math.round(aP[1])]
            }
            function aI(aS, aU, aR, aT) {
                var aW = aS,
                    aV = aR,
                    aQ = aU,
                    aP = aT;
            ...
            function aK() {
                ...
                if (p / ai && (Math.abs(aP) < p / ai)) {
                    aL = (aP > 0) ? (aM + p / ai) : (aM - p / ai)
                }
                if (m / aa && (Math.abs(aQ) < m / aa)) {
                    aA = (aQ > 0) ? (aB + m / aa) : (aB - m / aa)
                }
                if (aB < 0) {
                    aA -= aB;
                    aB -= aB
        ...
        var I = (function() {
            ...
            function aF() {
                return a("<div />").css({
                    position: "absolute",
                    backgroundColor: af.shadeColor || af.bgColor
                }).appendTo(aJ)
            }
            function aC() {
                if (!aE) {
                    ...
                    aJ.insertBefore(R);
                    aB();
                    ax.setBgOpacity(1, 0, 1);
                    ad.hide();
                    aI(af.shadeColor || af.bgColor, 1);
                    if (ax.isAwake()) {
                        aA(af.bgOpacity, 1)
                    } else {
                        aA(1, 1)
            ...
            function aK() {
                if (aE) {
                    ...
                    aE = false;
                    if (ax.isAwake()) {
                        ax.setBgOpacity(af.bgOpacity, 1, 1)
                    } else {
                        ax.setBgOpacity(1, 1, 1);
                        ax.disableHandles()
                    }
                    an(t, 0, 1)
                }
        ...
        var ax = (function() {
            ...
            function aH(a0) {
                var a1 = a("<div />").css({
                    position: "absolute",
                    opacity: af.borderOpacity
                }).addClass(V(a0));
                ay.append(a1);
                return a1
            }
            function aC(a0, a1) {
                var a2 = a("<div />").mousedown(aq(a0)).css({
                    cursor: a0 + "-resize",
                    position: "absolute",
                    zIndex: a1
                }).addClass("ord-" + a0);
                if (ar.support) {
                    a2.bind("touchstart.jcrop", ar.createDragger(a0))
                }
                Z.append(a2);
                return a2
            }
            function aL(a0) {
                var a1 = af.handleSize,
                    a2 = aC(a0, aT++).css({
                        opacity: af.handleOpacity
                    }).addClass(V("handle"));
                if (a1) {
                    a2.width(a1).height(a1)
                }
                return a2
            }
            function aR(a0) {
                return aC(a0, aT++).addClass("jcrop-dragbar")
            }
            function aO(a0) {
                var a1;
            ...
            function aZ(a0, a1) {
                M.width(Math.round(a0)).height(Math.round(a1))
            }
            function aE() {
                var a0 = k.getFixed();
                k.setPressed([a0.x, a0.y]);
                k.setCurrent([a0.x2, a0.y2]);
                aX()
            }
            function aX(a0) {
                if (aK) {
            ...
            function aI(a0) {
                ...
                aK || aY();
                if (a0) {
                    af.onSelect.call(E, q(a1))
                } else {
                    af.onChange.call(E, q(a1))
                }
            }
            function aA(a1, a2, a0) {
                if (!aK && !a2) {
                    return
                }
                if (af.bgFade && !a0) {
            ...
            if (af.dragEdges && a.isArray(af.createDragbars)) {
                aO(af.createDragbars)
            }
            if (a.isArray(af.createHandles)) {
                aN(af.createHandles)
            }
            if (af.drawBorders && a.isArray(af.createBorders)) {
                aS(af.createBorders)
            }
            a(document).bind("touchstart.jcrop-ios", function(a0) {
                if (a(a0.currentTarget).hasClass("jcrop-tracker")) {
                    a0.stopPropagation()
                }
            });
            var aF = at().mousedown(aq("move")).css({
                cursor: "move",
                position: "absolute",
                zIndex: 360
            });
            if (ar.support) {
                aF.bind("touchstart.jcrop", ar.createDragger("move"))
            }
            ay.append(aF);
            aV();
            return {
        ...
        var aj = (function() {
            ...
            function aJ(aK) {
                ...
                if (aK) {
                    a(document).bind("touchmove.jcrop", aI).bind("touchend.jcrop", aF)
                } else {
                    if (aB) {
                        a(document).bind("mousemove.jcrop", az).bind("mouseup.jcrop", aD)
                    }
                }
            }
            function aH() {
            ...
            function aD(aK) {
                ...
                if (g) {
                    ...
                    aC(ap(aK));
                    if (ax.isAwake()) {
                        af.onSelect.call(E, q(k.getFixed()))
                    }
                    aH();
                    aA = function() {};
                    aC = function() {}
            ...
            function aF(aK) {
                return aD(ar.cfilter(aK))
            }
            function aG(aK) {
                G.css("cursor", aK)
            }
            if (!aB) {
                G.mousemove(az).mouseup(aD).mouseout(aD)
            }
            R.before(G);
            return {
                activateHandlers: aE,
        ...
        var T = (function() {
            ...
            function aB(aG, aF, aH) {
                if (af.allowMove) {
                    k.moveOffset([aF, aH]);
                    ax.updateVisible(true)
                }
                aG.preventDefault();
                aG.stopPropagation()
            }
            ...
            if (af.keySupport) {
                aC.keydown(az).blur(aD);
                if (U || !af.fixedSupport) {
                    aC.css({
                        position: "absolute",
                        left: "-20px"
                    });
                    aE.append(aC).insertBefore(R)
                } else {
                    aC.insertBefore(R)
                }
            }
        ...
        function ah(az) {
            t.removeClass().addClass(V("holder")).addClass(az)
        }
        function S(aS, aG) {
            var aM = aS[0] / aa,
                aB = aS[1] / ai,
            ...
            var aK = k.flipCoords(aM, aB, aL, aA),
                aQ = k.getFixed(),
                aN = [aQ.x, aQ.y, aQ.x2, aQ.y2],
                aD = aN,
                aC = af.animationDelay,
                aP = aK[0] - aN[0],
                aF = aK[1] - aN[1],
                aO = aK[2] - aN[2],
                aE = aK[3] - aN[3],
            ...
            function aI() {
                window.setTimeout(aR, aC)
            }
            var aR = (function() {
                return function() {
                    aJ += (100 - aJ) / aH;
                    aD[0] = Math.round(aM + ((aJ / 100) * aP));
                    aD[1] = Math.round(aB + ((aJ / 100) * aF));
                    aD[2] = Math.round(aL + ((aJ / 100) * aO));
                    aD[3] = Math.round(aA + ((aJ / 100) * aE));
                    if (aJ >= 99.8) {
                        aJ = 100
                    }
                    if (aJ < 100) {
        ...
        function B(aA) {
            var az = parseInt(t.css("margin-left")) + parseInt(t.css("left"));
            var aB = parseInt(t.css("margin-top")) + parseInt(t.css("top"));
            al([(aA[0] - az) / aa, (aA[1] - aB) / ai, (aA[2] - az) / aa, (aA[3] - aB) / ai]);
            af.onSelect.call(E, q(k.getFixed()));
            ax.enableHandles()
        }
        function al(az) {
            k.setPressed([az[0], az[1]]);
            k.setCurrent([az[2], az[3]]);
            ax.update()
        }
        function am() {
            return q(k.getFixed())
        ...
        function j() {
            af.disabled = true;
            ax.disableHandles();
            ax.setCursor("default");
            aj.setCursor("default")
        }
        function w() {
            af.disabled = false;
            J()
        ...
        function W() {
            ax.done();
            aj.activateHandlers(null, null)
        }
        function h() {
            t.remove();
            u.show();
            u.css("visibility", "visible");
            a(f).removeData("Jcrop")
        }
        function ac(aA, aB) {
            ax.release();
            j();
        ...
        function an(aC, az, aA) {
            var aB = az || af.bgColor;
            if (af.bgFade && ab() && af.fadeTime && !aA) {
                aC.animate({
                    backgroundColor: aB
                }, {
                    queue: false,
        ...
        function J(az) {
            ...
            aj.setCursor(af.allowSelect ? "crosshair" : "default");
            ax.setCursor(af.allowMove ? "move" : "default");
            if (af.hasOwnProperty("trueSize")) {
                aa = af.trueSize[0] / n;
                ai = af.trueSize[1] / l
            }
            if (af.hasOwnProperty("setSelect")) {
                B(af.setSelect);
                ax.done();
                delete(af.setSelect)
            }
            I.refresh();
            if (af.bgColor != av) {
                an(af.shade ? I.getShades() : t, af.shade ? (af.shadeColor || af.bgColor) : af.bgColor);
                av = af.bgColor
            }
            if (C != af.bgOpacity) {
                C = af.bgOpacity;
            ...
            m = af.minSize[0] || 0;
            p = af.minSize[1] || 0;
            if (af.hasOwnProperty("outerImage")) {
                R.attr("src", af.outerImage);
                delete(af.outerImage)
            }
            ax.refresh()
        }
        if (ar.support) {
            G.bind("touchstart.jcrop", ar.newSelection)
        }
        Z.hide();
        J(true);
        var E = {
...
(function(g) {
    ...
    (function(j) {
        ...
        j.browser = j.browser || {};
        j.browser.opera = /opera(\/| )(\d+(\.\d+)?)(.+?(version\/(\d+(\.\d+)?)))?/i.test(navigator.userAgent) ? +(RegExp["$6"] || RegExp["$2"]) : undefined;
    ...
    var a = function(j) {
        ...
        this.onSaveSuccess = j.onSaveSuccess ||
            function() {};
        this.onSaveError = j.onSaveError ||
            function() {};
        this.onShow = j.onShow ||
            function() {};
        this.onHide = j.onHide ||
            function() {}
    };
    a.prototype = {
        init: function(k) {
